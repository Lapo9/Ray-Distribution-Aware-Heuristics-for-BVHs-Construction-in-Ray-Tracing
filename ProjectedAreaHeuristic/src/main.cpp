...
#define and_or &&
	...
	constexpr bool PLANE_FULL_STANDARD = ALL || false;
	constexpr bool POINT_FULL_STANDARD = ALL || false;
	...
	{
		// === Wood scene: 1 point full influence area ===
		if constexpr (POINT_FULL_LONGEST and_or WOOD_SCENE) {
			// === Wood scene: 1 point influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PointInfluenceArea influenceAreaPointFullParallel{ Pov{{2.5,.9,0}, {-1,0,0}, 90, 60}, 10, 1, 10000 };
				PointRayCaster rayCasterPointFullParallel{ influenceAreaPointFullParallel }; rayCasterPointFullParallel.generateRays(rng, 1000, true);
		...
		// === Suzanne scene: 1 point full influence area ===
		if constexpr (POINT_FULL_LONGEST and_or SUZANNE_SCENE) {
			// === Suzanne scene: 1 point influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PointInfluenceArea influenceAreaPointFullParallel{ Pov{{0,0,4}, {0,0,-1}, 30, 30}, 10, 2, 10000 };
				PointRayCaster rayCasterPointFullParallel{ influenceAreaPointFullParallel }; rayCasterPointFullParallel.generateRays(rng, 1000, true);
		...
		// === Cottage scene: 1 point full influence area ===
		if constexpr (POINT_FULL_LONGEST and_or COTTAGE_SCENE) {
			// === Cottage scene: 1 point influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PointInfluenceArea influenceAreaPointFullParallel{ Pov{{15,2.7,0}, {-1,0,0}, 70, 35}, 35, 1, 10000 };
				PointRayCaster rayCasterPointFullParallel{ influenceAreaPointFullParallel }; rayCasterPointFullParallel.generateRays(rng, 1000, true);
		...
		// === CottageWalls scene: 1 point full influence area ===
		if constexpr (POINT_FULL_LONGEST and_or COTTAGE_WALLS_SCENE) {
			// === CottageWalls scene: 1 point influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PointInfluenceArea influenceAreaPointFullParallel{ Pov{{15,2.7,0}, {-1,0,0}, 70, 35}, 35, 1, 10000 };
				PointRayCaster rayCasterPointFullParallel{ influenceAreaPointFullParallel }; rayCasterPointFullParallel.generateRays(rng, 1000, true);
		...
		// === Crowd scene: 1 point full influence area ===
		if constexpr (POINT_FULL_LONGEST and_or CROWD_SCENE) {
			// === Crowd scene: 1 point influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PointInfluenceArea influenceAreaPointFullParallel{ Pov{{2,2.2,-8}, {0,0,1}, 130, 45}, 30, 1, 10000 };
				PointRayCaster rayCasterPointFullParallel{ influenceAreaPointFullParallel }; rayCasterPointFullParallel.generateRays(rng, 1000, true);
		...
		// === Random100 scene: 1 point full influence area ===
		if constexpr (POINT_FULL_LONGEST and_or RANDOM100_SCENE) {
			// === Random100 scene: 1 point influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PointInfluenceArea influenceAreaPointFullParallel{ Pov{{5,5,-4}, {0,0,1}, 90, 90}, 20, 1, 10000 };
				PointRayCaster rayCasterPointFullParallel{ influenceAreaPointFullParallel }; rayCasterPointFullParallel.generateRays(rng, 1000, true);
		...
		// === Random1000 scene: 1 point full influence area ===
		if constexpr (POINT_FULL_LONGEST and_or RANDOM1000_SCENE) {
			// === Random1000 scene: 1 point influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PointInfluenceArea influenceAreaPointFullParallel{ Pov{{5,5,-4}, {0,0,1}, 90, 90}, 20, 1, 10000 };
				PointRayCaster rayCasterPointFullParallel{ influenceAreaPointFullParallel }; rayCasterPointFullParallel.generateRays(rng, 1000, true);
		...
		// === Sponza scene: 1 point full influence area ===
		if constexpr (POINT_FULL_LONGEST and_or SPONZA_SCENE) {
			// === Sponza scene: 1 point influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PointInfluenceArea influenceAreaPointFullParallel{ Pov{{-9,11,-1}, {1,0,0}, 70, 50}, 50, 1, 10000 };
				PointRayCaster rayCasterPointFullParallel{ influenceAreaPointFullParallel }; rayCasterPointFullParallel.generateRays(rng, 1000, true);
	...
	bvhProperties.splitPlaneQualityThreshold = 0.1f;
	{
		...
		if constexpr (PLANE_FULL_SAH and_or WOOD_SCENE) {
			// === Wood scene: 1 plane influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFullParallel{ Plane{{2.5,.9,0}, {-1,0,0}, 1.5,0.9}, 10, 10000 };
				PlaneRayCaster rayCasterPlaneFullParallel{ influenceAreaPlaneFullParallel }; rayCasterPlaneFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPlaneFullParallel{ bvhProperties, influenceAreaPlaneFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "WoodPlaneFullSahParallel", woodTriangles, vector{&rayCasterPlaneFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("WoodPlaneFullSahParallel", scene.buildAndTraverse());
			}
			...
			// === Wood scene: 1 plane influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFull15{ Plane{{2.5,.8,-.3}, {-.966,0,.259}, 1.6,1}, 10, 10000 };
				PlaneRayCaster rayCasterPlaneFull15{ influenceAreaPlaneFull15 }; rayCasterPlaneFull15.generateRays(rng, 1000, true);
				Bvh bvhPlaneFull15{ bvhProperties, influenceAreaPlaneFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "WoodPlaneFullSah15", woodTriangles, vector{&rayCasterPlaneFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("WoodPlaneFullSah15", scene.buildAndTraverse());
			}
			...
			// === Wood scene: 1 plane influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFull45{ Plane{{2.5,.8,-3}, {-1,0,1}, 1.5,1}, 10, 10000 };
				PlaneRayCaster rayCasterPlaneFull45{ influenceAreaPlaneFull45 }; rayCasterPlaneFull45.generateRays(rng, 1000, true);
				Bvh bvhPlaneFull45{ bvhProperties, influenceAreaPlaneFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "WoodPlaneFullSah45", woodTriangles, vector{&rayCasterPlaneFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("WoodPlaneFullSah45", scene.buildAndTraverse());
			}
			...
			// === Wood scene: 1 plane influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFullOblique{ Plane{{2.5,1.5,-1}, {-1,-0.3,0.4}, 1.8,1.3}, 20, 10000 };
				PlaneRayCaster rayCasterPlaneFullOblique{ influenceAreaPlaneFullOblique }; rayCasterPlaneFullOblique.generateRays(rng, 1000, true);
				Bvh bvhPlaneFullOblique{ bvhProperties, influenceAreaPlaneFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "WoodPlaneFullSahOblique", woodTriangles, vector{&rayCasterPlaneFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("WoodPlaneFullSahOblique", scene.buildAndTraverse());
			}
		}

		// === Suzanne scene: 1 plane full influence area ===
		if constexpr (PLANE_FULL_SAH and_or SUZANNE_SCENE) {
			// === Suzanne scene: 1 plane influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFullParallel{ Plane{{0,0,2}, {0,0,-1}, .9,.9}, 10, 10000 };
				PlaneRayCaster rayCasterPlaneFullParallel{ influenceAreaPlaneFullParallel }; rayCasterPlaneFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPlaneFullParallel{ bvhProperties, influenceAreaPlaneFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SuzannePlaneFullSahParallel", suzanneTriangles, vector{&rayCasterPlaneFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SuzannePlaneFullSahParallel", scene.buildAndTraverse());
			}
			...
			// === Suzanne scene: 1 plane influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull15{ bvhProperties, influenceAreaPlaneFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SuzannePlaneFullSah15", suzanneTriangles, vector{&rayCasterPlaneFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SuzannePlaneFullSah15", scene.buildAndTraverse());
			}
			...
			// === Suzanne scene: 1 plane influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull45{ bvhProperties, influenceAreaPlaneFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SuzannePlaneFullSah45", suzanneTriangles, vector{&rayCasterPlaneFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SuzannePlaneFullSah45", scene.buildAndTraverse());
			}
			...
			// === Suzanne scene: 1 plane influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFullOblique{ bvhProperties, influenceAreaPlaneFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SuzannePlaneFullSahOblique", suzanneTriangles, vector{&rayCasterPlaneFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SuzannePlaneFullSahOblique", scene.buildAndTraverse());
			}
		}

		// === Cottage scene: 1 plane full influence area ===
		if constexpr (PLANE_FULL_SAH and_or COTTAGE_SCENE) {
			// === Cottage scene: 1 plane influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFullParallel{ Plane{{10,2.8,0}, {-1,0,0}, 6,2.6}, 35, 10000 };
				PlaneRayCaster rayCasterPlaneFullParallel{ influenceAreaPlaneFullParallel }; rayCasterPlaneFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPlaneFullParallel{ bvhProperties, influenceAreaPlaneFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottagePlaneFullSahParallel", cottageTriangles, vector{&rayCasterPlaneFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottagePlaneFullSahParallel", scene.buildAndTraverse());
			}
			...
			// === Cottage scene: 1 plane influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull15{ bvhProperties, influenceAreaPlaneFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottagePlaneFullSah15", cottageTriangles, vector{&rayCasterPlaneFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottagePlaneFullSah15", scene.buildAndTraverse());
			}
			...
			// === Cottage scene: 1 plane influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull45{ bvhProperties, influenceAreaPlaneFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottagePlaneFullSah45", cottageTriangles, vector{&rayCasterPlaneFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottagePlaneFullSah45", scene.buildAndTraverse());
			}
			...
			// === Cottage scene: 1 plane influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFullOblique{ bvhProperties, influenceAreaPlaneFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottagePlaneFullSahOblique", cottageTriangles, vector{&rayCasterPlaneFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottagePlaneFullSahOblique", scene.buildAndTraverse());
			}
		}

		// === CottageWalls scene: 1 plane full influence area ===
		if constexpr (PLANE_FULL_SAH and_or COTTAGE_WALLS_SCENE) {
			// === CottageWalls scene: 1 plane influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFullParallel{ Plane{{10,2.8,0}, {-1,0,0}, 6,2.6}, 35, 10000 };
				PlaneRayCaster rayCasterPlaneFullParallel{ influenceAreaPlaneFullParallel }; rayCasterPlaneFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPlaneFullParallel{ bvhProperties, influenceAreaPlaneFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottageWallsPlaneFullSahParallel", cottageWallsTriangles, vector{&rayCasterPlaneFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottageWallsPlaneFullSahParallel", scene.buildAndTraverse());
			}
			...
			// === CottageWalls scene: 1 plane influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull15{ bvhProperties, influenceAreaPlaneFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottageWallsPlaneFullSah15", cottageWallsTriangles, vector{&rayCasterPlaneFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottageWallsPlaneFullSah15", scene.buildAndTraverse());
			}
			...
			// === CottageWalls scene: 1 plane influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull45{ bvhProperties, influenceAreaPlaneFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottageWallsPlaneFullSah45", cottageWallsTriangles, vector{&rayCasterPlaneFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottageWallsPlaneFullSah45", scene.buildAndTraverse());
			}
			...
			// === CottageWalls scene: 1 plane influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFullOblique{ bvhProperties, influenceAreaPlaneFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottageWallsPlaneFullSahOblique", cottageWallsTriangles, vector{&rayCasterPlaneFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottageWallsPlaneFullSahOblique", scene.buildAndTraverse());
			}
		}

		// === Crowd scene: 1 plane full influence area ===
		if constexpr (PLANE_FULL_SAH and_or CROWD_SCENE) {
			// === Crowd scene: 1 plane influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFullParallel{ Plane{{2,2.2,-8}, {0,0,1}, 7,1.8}, 30, 10000 };
				PlaneRayCaster rayCasterPlaneFullParallel{ influenceAreaPlaneFullParallel }; rayCasterPlaneFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPlaneFullParallel{ bvhProperties, influenceAreaPlaneFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CrowdPlaneFullSahParallel", crowdTriangles, vector{&rayCasterPlaneFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CrowdPlaneFullSahParallel", scene.buildAndTraverse());
			}
			...
			// === Crowd scene: 1 plane influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull15{ bvhProperties, influenceAreaPlaneFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CrowdPlaneFullSah15", crowdTriangles, vector{&rayCasterPlaneFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CrowdPlaneFullSah15", scene.buildAndTraverse());
			}
			...
			// === Crowd scene: 1 plane influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull45{ bvhProperties, influenceAreaPlaneFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CrowdPlaneFullSah45", crowdTriangles, vector{&rayCasterPlaneFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CrowdPlaneFullSah45", scene.buildAndTraverse());
			}
			...
			// === Crowd scene: 1 plane influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFullOblique{ bvhProperties, influenceAreaPlaneFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CrowdPlaneFullSahOblique", crowdTriangles, vector{&rayCasterPlaneFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CrowdPlaneFullSahOblique", scene.buildAndTraverse());
			}
		}

		// === Random100 scene: 1 plane full influence area ===
		if constexpr (PLANE_FULL_SAH and_or RANDOM100_SCENE) {
			// === Random100 scene: 1 plane influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFullParallel{ Plane{{5,5,-4}, {0,0,1}, 5,5}, 20, 10000 };
				PlaneRayCaster rayCasterPlaneFullParallel{ influenceAreaPlaneFullParallel }; rayCasterPlaneFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPlaneFullParallel{ bvhProperties, influenceAreaPlaneFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random100PlaneFullSahParallel", random100Triangles, vector{&rayCasterPlaneFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random100PlaneFullSahParallel", scene.buildAndTraverse());
			}
			...
			// === Random100 scene: 1 plane influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull15{ bvhProperties, influenceAreaPlaneFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random100PlaneFullSah15", random100Triangles, vector{&rayCasterPlaneFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random100PlaneFullSah15", scene.buildAndTraverse());
			}
			...
			// === Random100 scene: 1 plane influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull45{ bvhProperties, influenceAreaPlaneFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random100PlaneFullSah45", random100Triangles, vector{&rayCasterPlaneFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random100PlaneFullSah45", scene.buildAndTraverse());
			}
			...
			// === Random100 scene: 1 plane influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFullOblique{ bvhProperties, influenceAreaPlaneFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random100PlaneFullSahOblique", random100Triangles, vector{&rayCasterPlaneFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random100PlaneFullSahOblique", scene.buildAndTraverse());
			}
		}

		// === Random1000 scene: 1 plane full influence area ===
		if constexpr (PLANE_FULL_SAH and_or RANDOM1000_SCENE) {
			// === Random1000 scene: 1 plane influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFullParallel{ Plane{{5,5,-4}, {0,0,1}, 5,5}, 20, 10000 };
				PlaneRayCaster rayCasterPlaneFullParallel{ influenceAreaPlaneFullParallel }; rayCasterPlaneFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPlaneFullParallel{ bvhProperties, influenceAreaPlaneFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random1000PlaneFullSahParallel", random1000Triangles, vector{&rayCasterPlaneFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random1000PlaneFullSahParallel", scene.buildAndTraverse());
			}
			...
			// === Random1000 scene: 1 plane influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull15{ bvhProperties, influenceAreaPlaneFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random1000PlaneFullSah15", random1000Triangles, vector{&rayCasterPlaneFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random1000PlaneFullSah15", scene.buildAndTraverse());
			}
			...
			// === Random1000 scene: 1 plane influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull45{ bvhProperties, influenceAreaPlaneFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random1000PlaneFullSah45", random1000Triangles, vector{&rayCasterPlaneFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random1000PlaneFullSah45", scene.buildAndTraverse());
			}
			...
			// === Random1000 scene: 1 plane influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFullOblique{ bvhProperties, influenceAreaPlaneFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random1000PlaneFullSahOblique", random1000Triangles, vector{&rayCasterPlaneFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random1000PlaneFullSahOblique", scene.buildAndTraverse());
			}
		}

		// === Sponza scene: 1 plane full influence area ===
		if constexpr (PLANE_FULL_SAH and_or SPONZA_SCENE) {
			// === Sponza scene: 1 plane influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFullParallel{ Plane{{-9,11,-1}, {1,0,0}, 16, 11}, 50, 10000 };
				PlaneRayCaster rayCasterPlaneFullParallel{ influenceAreaPlaneFullParallel }; rayCasterPlaneFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPlaneFullParallel{ bvhProperties, influenceAreaPlaneFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SponzaPlaneFullSahParallel", sponzaTriangles, vector{&rayCasterPlaneFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SponzaPlaneFullSahParallel", scene.buildAndTraverse());
			}
			...
			// === Sponza scene: 1 plane influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull15{ bvhProperties, influenceAreaPlaneFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SponzaPlaneFullSah15", sponzaTriangles, vector{&rayCasterPlaneFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SponzaPlaneFullSah15", scene.buildAndTraverse());
			}
			...
			// === Sponza scene: 1 plane influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull45{ bvhProperties, influenceAreaPlaneFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesFacing, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SponzaPlaneFullSah45", sponzaTriangles, vector{&rayCasterPlaneFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SponzaPlaneFullSah45", scene.buildAndTraverse());
			}
	...
	{
		// === Wood scene: 1 plane full influence area ===
		if constexpr (PLANE_FULL_STANDARD and_or WOOD_SCENE) {
			// === Wood scene: 1 plane influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFullParallel{ Plane{{2.5,.9,0}, {-1,0,0}, 1.5,0.9}, 10, 10000 };
				PlaneRayCaster rayCasterPlaneFullParallel{ influenceAreaPlaneFullParallel }; rayCasterPlaneFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPlaneFullParallel{ bvhProperties, influenceAreaPlaneFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "WoodPlaneFullStandardParallel", woodTriangles, vector{&rayCasterPlaneFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("WoodPlaneFullStandardParallel", scene.buildAndTraverse());
			}
			...
			// === Wood scene: 1 plane influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFull15{ Plane{{2.5,.8,-.3}, {-.966,0,.259}, 1.6,1}, 10, 10000 };
				PlaneRayCaster rayCasterPlaneFull15{ influenceAreaPlaneFull15 }; rayCasterPlaneFull15.generateRays(rng, 1000, true);
				Bvh bvhPlaneFull15{ bvhProperties, influenceAreaPlaneFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "WoodPlaneFullStandard15", woodTriangles, vector{&rayCasterPlaneFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("WoodPlaneFullStandard15", scene.buildAndTraverse());
			}
			...
			// === Wood scene: 1 plane influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFull45{ Plane{{2.5,.8,-3}, {-1,0,1}, 1.5,1}, 10, 10000 };
				PlaneRayCaster rayCasterPlaneFull45{ influenceAreaPlaneFull45 }; rayCasterPlaneFull45.generateRays(rng, 1000, true);
				Bvh bvhPlaneFull45{ bvhProperties, influenceAreaPlaneFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "WoodPlaneFullStandard45", woodTriangles, vector{&rayCasterPlaneFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("WoodPlaneFullStandard45", scene.buildAndTraverse());
			}
			...
			// === Wood scene: 1 plane influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFullOblique{ Plane{{2.5,1.5,-1}, {-1,-0.3,0.4}, 1.8,1.3}, 20, 10000 };
				PlaneRayCaster rayCasterPlaneFullOblique{ influenceAreaPlaneFullOblique }; rayCasterPlaneFullOblique.generateRays(rng, 1000, true);
				Bvh bvhPlaneFullOblique{ bvhProperties, influenceAreaPlaneFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "WoodPlaneFullStandardOblique", woodTriangles, vector{&rayCasterPlaneFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("WoodPlaneFullStandardOblique", scene.buildAndTraverse());
			}
		}

		// === Suzanne scene: 1 plane full influence area ===
		if constexpr (PLANE_FULL_STANDARD and_or SUZANNE_SCENE) {
			// === Suzanne scene: 1 plane influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFullParallel{ Plane{{0,0,2}, {0,0,-1}, .9,.9}, 10, 10000 };
				PlaneRayCaster rayCasterPlaneFullParallel{ influenceAreaPlaneFullParallel }; rayCasterPlaneFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPlaneFullParallel{ bvhProperties, influenceAreaPlaneFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SuzannePlaneFullStandardParallel", suzanneTriangles, vector{&rayCasterPlaneFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SuzannePlaneFullStandardParallel", scene.buildAndTraverse());
			}
			...
			// === Suzanne scene: 1 plane influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull15{ bvhProperties, influenceAreaPlaneFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SuzannePlaneFullStandard15", suzanneTriangles, vector{&rayCasterPlaneFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SuzannePlaneFullStandard15", scene.buildAndTraverse());
			}
			...
			// === Suzanne scene: 1 plane influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull45{ bvhProperties, influenceAreaPlaneFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SuzannePlaneFullStandard45", suzanneTriangles, vector{&rayCasterPlaneFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SuzannePlaneFullStandard45", scene.buildAndTraverse());
			}
			...
			// === Suzanne scene: 1 plane influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFullOblique{ bvhProperties, influenceAreaPlaneFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SuzannePlaneFullStandardOblique", suzanneTriangles, vector{&rayCasterPlaneFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SuzannePlaneFullStandardOblique", scene.buildAndTraverse());
			}
		}

		// === Cottage scene: 1 plane full influence area ===
		if constexpr (PLANE_FULL_STANDARD and_or COTTAGE_SCENE) {
			// === Cottage scene: 1 plane influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFullParallel{ Plane{{10,2.8,0}, {-1,0,0}, 6,2.6}, 35, 10000 };
				PlaneRayCaster rayCasterPlaneFullParallel{ influenceAreaPlaneFullParallel }; rayCasterPlaneFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPlaneFullParallel{ bvhProperties, influenceAreaPlaneFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottagePlaneFullStandardParallel", cottageTriangles, vector{&rayCasterPlaneFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottagePlaneFullStandardParallel", scene.buildAndTraverse());
			}
			...
			// === Cottage scene: 1 plane influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull15{ bvhProperties, influenceAreaPlaneFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottagePlaneFullStandard15", cottageTriangles, vector{&rayCasterPlaneFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottagePlaneFullStandard15", scene.buildAndTraverse());
			}
			...
			// === Cottage scene: 1 plane influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull45{ bvhProperties, influenceAreaPlaneFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottagePlaneFullStandard45", cottageTriangles, vector{&rayCasterPlaneFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottagePlaneFullStandard45", scene.buildAndTraverse());
			}
			...
			// === Cottage scene: 1 plane influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFullOblique{ bvhProperties, influenceAreaPlaneFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottagePlaneFullStandardOblique", cottageTriangles, vector{&rayCasterPlaneFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottagePlaneFullStandardOblique", scene.buildAndTraverse());
			}
		}

		// === CottageWalls scene: 1 plane full influence area ===
		if constexpr (PLANE_FULL_STANDARD and_or COTTAGE_WALLS_SCENE) {
			// === CottageWalls scene: 1 plane influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFullParallel{ Plane{{10,2.8,0}, {-1,0,0}, 6,2.6}, 35, 10000 };
				PlaneRayCaster rayCasterPlaneFullParallel{ influenceAreaPlaneFullParallel }; rayCasterPlaneFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPlaneFullParallel{ bvhProperties, influenceAreaPlaneFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottageWallsPlaneFullStandardParallel", cottageWallsTriangles, vector{&rayCasterPlaneFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottageWallsPlaneFullStandardParallel", scene.buildAndTraverse());
			}
			...
			// === CottageWalls scene: 1 plane influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull15{ bvhProperties, influenceAreaPlaneFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottageWallsPlaneFullStandard15", cottageWallsTriangles, vector{&rayCasterPlaneFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottageWallsPlaneFullStandard15", scene.buildAndTraverse());
			}
			...
			// === CottageWalls scene: 1 plane influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull45{ bvhProperties, influenceAreaPlaneFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottageWallsPlaneFullStandard45", cottageWallsTriangles, vector{&rayCasterPlaneFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottageWallsPlaneFullStandard45", scene.buildAndTraverse());
			}
			...
			// === CottageWalls scene: 1 plane influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFullOblique{ bvhProperties, influenceAreaPlaneFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottageWallsPlaneFullStandardOblique", cottageWallsTriangles, vector{&rayCasterPlaneFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottageWallsPlaneFullStandardOblique", scene.buildAndTraverse());
			}
		}

		// === Crowd scene: 1 plane full influence area ===
		if constexpr (PLANE_FULL_STANDARD and_or CROWD_SCENE) {
			// === Crowd scene: 1 plane influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFullParallel{ Plane{{2,2.2,-8}, {0,0,1}, 7,1.8}, 30, 10000 };
				PlaneRayCaster rayCasterPlaneFullParallel{ influenceAreaPlaneFullParallel }; rayCasterPlaneFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPlaneFullParallel{ bvhProperties, influenceAreaPlaneFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CrowdPlaneFullStandardParallel", crowdTriangles, vector{&rayCasterPlaneFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CrowdPlaneFullStandardParallel", scene.buildAndTraverse());
			}
			...
			// === Crowd scene: 1 plane influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull15{ bvhProperties, influenceAreaPlaneFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CrowdPlaneFullStandard15", crowdTriangles, vector{&rayCasterPlaneFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CrowdPlaneFullStandard15", scene.buildAndTraverse());
			}
			...
			// === Crowd scene: 1 plane influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull45{ bvhProperties, influenceAreaPlaneFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CrowdPlaneFullStandard45", crowdTriangles, vector{&rayCasterPlaneFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CrowdPlaneFullStandard45", scene.buildAndTraverse());
			}
			...
			// === Crowd scene: 1 plane influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFullOblique{ bvhProperties, influenceAreaPlaneFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CrowdPlaneFullStandardOblique", crowdTriangles, vector{&rayCasterPlaneFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CrowdPlaneFullStandardOblique", scene.buildAndTraverse());
			}
		}

		// === Random100 scene: 1 plane full influence area ===
		if constexpr (PLANE_FULL_STANDARD and_or RANDOM100_SCENE) {
			// === Random100 scene: 1 plane influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFullParallel{ Plane{{5,5,-4}, {0,0,1}, 5,5}, 20, 10000 };
				PlaneRayCaster rayCasterPlaneFullParallel{ influenceAreaPlaneFullParallel }; rayCasterPlaneFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPlaneFullParallel{ bvhProperties, influenceAreaPlaneFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random100PlaneFullStandardParallel", random100Triangles, vector{&rayCasterPlaneFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random100PlaneFullStandardParallel", scene.buildAndTraverse());
			}
			...
			// === Random100 scene: 1 plane influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull15{ bvhProperties, influenceAreaPlaneFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random100PlaneFullStandard15", random100Triangles, vector{&rayCasterPlaneFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random100PlaneFullStandard15", scene.buildAndTraverse());
			}
			...
			// === Random100 scene: 1 plane influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull45{ bvhProperties, influenceAreaPlaneFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random100PlaneFullStandard45", random100Triangles, vector{&rayCasterPlaneFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random100PlaneFullStandard45", scene.buildAndTraverse());
			}
			...
			// === Random100 scene: 1 plane influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFullOblique{ bvhProperties, influenceAreaPlaneFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random100PlaneFullStandardOblique", random100Triangles, vector{&rayCasterPlaneFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random100PlaneFullStandardOblique", scene.buildAndTraverse());
			}
		}

		// === Random1000 scene: 1 plane full influence area ===
		if constexpr (PLANE_FULL_STANDARD and_or RANDOM1000_SCENE) {
			// === Random1000 scene: 1 plane influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFullParallel{ Plane{{5,5,-4}, {0,0,1}, 5,5}, 20, 10000 };
				PlaneRayCaster rayCasterPlaneFullParallel{ influenceAreaPlaneFullParallel }; rayCasterPlaneFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPlaneFullParallel{ bvhProperties, influenceAreaPlaneFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random1000PlaneFullStandardParallel", random1000Triangles, vector{&rayCasterPlaneFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random1000PlaneFullStandardParallel", scene.buildAndTraverse());
			}
			...
			// === Random1000 scene: 1 plane influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull15{ bvhProperties, influenceAreaPlaneFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random1000PlaneFullStandard15", random1000Triangles, vector{&rayCasterPlaneFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random1000PlaneFullStandard15", scene.buildAndTraverse());
			}
			...
			// === Random1000 scene: 1 plane influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull45{ bvhProperties, influenceAreaPlaneFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random1000PlaneFullStandard45", random1000Triangles, vector{&rayCasterPlaneFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random1000PlaneFullStandard45", scene.buildAndTraverse());
			}
			...
			// === Random1000 scene: 1 plane influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFullOblique{ bvhProperties, influenceAreaPlaneFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random1000PlaneFullStandardOblique", random1000Triangles, vector{&rayCasterPlaneFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random1000PlaneFullStandardOblique", scene.buildAndTraverse());
			}
		}

		// === Sponza scene: 1 plane full influence area ===
		if constexpr (PLANE_FULL_STANDARD and_or SPONZA_SCENE) {
			// === Sponza scene: 1 plane influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PlaneInfluenceArea influenceAreaPlaneFullParallel{ Plane{{-9,11,-1}, {1,0,0}, 16, 11}, 50, 10000 };
				PlaneRayCaster rayCasterPlaneFullParallel{ influenceAreaPlaneFullParallel }; rayCasterPlaneFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPlaneFullParallel{ bvhProperties, influenceAreaPlaneFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SponzaPlaneFullStandardParallel", sponzaTriangles, vector{&rayCasterPlaneFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SponzaPlaneFullStandardParallel", scene.buildAndTraverse());
			}
			...
			// === Sponza scene: 1 plane influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull15{ bvhProperties, influenceAreaPlaneFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SponzaPlaneFullStandard15", sponzaTriangles, vector{&rayCasterPlaneFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SponzaPlaneFullStandard15", scene.buildAndTraverse());
			}
			...
			// === Sponza scene: 1 plane influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFull45{ bvhProperties, influenceAreaPlaneFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SponzaPlaneFullStandard45", sponzaTriangles, vector{&rayCasterPlaneFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SponzaPlaneFullStandard45", scene.buildAndTraverse());
			}
			...
			// === Sponza scene: 1 plane influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPlaneFullOblique{ bvhProperties, influenceAreaPlaneFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "plane" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPlaneFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SponzaPlaneFullStandardOblique", sponzaTriangles, vector{&rayCasterPlaneFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SponzaPlaneFullStandardOblique", scene.buildAndTraverse());
			}
		}
	};
	...
	{
		// === Wood scene: 1 point full influence area ===
		if constexpr (POINT_FULL_STANDARD and_or WOOD_SCENE) {
			// === Wood scene: 1 point influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PointInfluenceArea influenceAreaPointFullParallel{ Pov{{2.5,.9,0}, {-1,0,0}, 90, 60}, 10, 1, 10000 };
				PointRayCaster rayCasterPointFullParallel{ influenceAreaPointFullParallel }; rayCasterPointFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPointFullParallel{ bvhProperties, influenceAreaPointFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "WoodPointFullStandardParallel", woodTriangles, vector{&rayCasterPointFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("WoodPointFullStandardParallel", scene.buildAndTraverse());
			}
			...
			// === Wood scene: 1 point influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PointInfluenceArea influenceAreaPointFull15{ Pov{{2.5,.8,-.3}, {-.966,0,.259}, 90, 60}, 10, 1, 10000 };
				PointRayCaster rayCasterPointFull15{ influenceAreaPointFull15 }; rayCasterPointFull15.generateRays(rng, 1000, true);
				Bvh bvhPointFull15{ bvhProperties, influenceAreaPointFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "WoodPointFullStandard15", woodTriangles, vector{&rayCasterPointFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("WoodPointFullStandard15", scene.buildAndTraverse());
			}
			...
			// === Wood scene: 1 point influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PointInfluenceArea influenceAreaPointFull45{ Pov{{2.5,.8,-3}, {-1,0,1}, 90, 60}, 10, 1, 10000 };
				PointRayCaster rayCasterPointFull45{ influenceAreaPointFull45 }; rayCasterPointFull45.generateRays(rng, 1000, true);
				Bvh bvhPointFull45{ bvhProperties, influenceAreaPointFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "WoodPointFullStandard45", woodTriangles, vector{&rayCasterPointFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("WoodPointFullStandard45", scene.buildAndTraverse());
			}
			...
			// === Wood scene: 1 point influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PointInfluenceArea influenceAreaPointFullOblique{ Pov{{2.5,1.5,-1}, {-1,-0.3,0.4}, 90, 60}, 20, 1, 10000 };
				PointRayCaster rayCasterPointFullOblique{ influenceAreaPointFullOblique }; rayCasterPointFullOblique.generateRays(rng, 1000, true);
				Bvh bvhPointFullOblique{ bvhProperties, influenceAreaPointFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "WoodPointFullStandardOblique", woodTriangles, vector{&rayCasterPointFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("WoodPointFullStandardOblique", scene.buildAndTraverse());
			}
		}

		// === Suzanne scene: 1 point full influence area ===
		if constexpr (POINT_FULL_STANDARD and_or SUZANNE_SCENE) {
			// === Suzanne scene: 1 point influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PointInfluenceArea influenceAreaPointFullParallel{ Pov{{0,0,4}, {0,0,-1}, 30, 30}, 10, 2, 10000 };
				PointRayCaster rayCasterPointFullParallel{ influenceAreaPointFullParallel }; rayCasterPointFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPointFullParallel{ bvhProperties, influenceAreaPointFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SuzannePointFullStandardParallel", suzanneTriangles, vector{&rayCasterPointFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SuzannePointFullStandardParallel", scene.buildAndTraverse());
			}
			...
			// === Suzanne scene: 1 point influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFull15{ bvhProperties, influenceAreaPointFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SuzannePointFullStandard15", suzanneTriangles, vector{&rayCasterPointFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SuzannePointFullStandard15", scene.buildAndTraverse());
			}
			...
			// === Suzanne scene: 1 point influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFull45{ bvhProperties, influenceAreaPointFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SuzannePointFullStandard45", suzanneTriangles, vector{&rayCasterPointFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SuzannePointFullStandard45", scene.buildAndTraverse());
			}
			...
			// === Suzanne scene: 1 point influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFullOblique{ bvhProperties, influenceAreaPointFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SuzannePointFullStandardOblique", suzanneTriangles, vector{&rayCasterPointFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SuzannePointFullStandardOblique", scene.buildAndTraverse());
			}
		}

		// === Cottage scene: 1 point full influence area ===
		if constexpr (POINT_FULL_STANDARD and_or COTTAGE_SCENE) {
			// === Cottage scene: 1 point influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PointInfluenceArea influenceAreaPointFullParallel{ Pov{{15,2.7,0}, {-1,0,0}, 70, 35}, 35, 1, 10000 };
				PointRayCaster rayCasterPointFullParallel{ influenceAreaPointFullParallel }; rayCasterPointFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPointFullParallel{ bvhProperties, influenceAreaPointFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottagePointFullStandardParallel", cottageTriangles, vector{&rayCasterPointFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottagePointFullStandardParallel", scene.buildAndTraverse());
			}
			...
			// === Cottage scene: 1 point influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFull15{ bvhProperties, influenceAreaPointFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottagePointFullStandard15", cottageTriangles, vector{&rayCasterPointFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottagePointFullStandard15", scene.buildAndTraverse());
			}
			...
			// === Cottage scene: 1 point influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFull45{ bvhProperties, influenceAreaPointFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottagePointFullStandard45", cottageTriangles, vector{&rayCasterPointFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottagePointFullStandard45", scene.buildAndTraverse());
			}
			...
			// === Cottage scene: 1 point influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFullOblique{ bvhProperties, influenceAreaPointFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottagePointFullStandardOblique", cottageTriangles, vector{&rayCasterPointFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottagePointFullStandardOblique", scene.buildAndTraverse());
			}
		}

		// === CottageWalls scene: 1 point full influence area ===
		if constexpr (POINT_FULL_STANDARD and_or COTTAGE_WALLS_SCENE) {
			// === CottageWalls scene: 1 point influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PointInfluenceArea influenceAreaPointFullParallel{ Pov{{15,2.7,0}, {-1,0,0}, 70, 35}, 35, 1, 10000 };
				PointRayCaster rayCasterPointFullParallel{ influenceAreaPointFullParallel }; rayCasterPointFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPointFullParallel{ bvhProperties, influenceAreaPointFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottageWallsPointFullStandardParallel", cottageWallsTriangles, vector{&rayCasterPointFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottageWallsPointFullStandardParallel", scene.buildAndTraverse());
			}
			...
			// === CottageWalls scene: 1 point influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFull15{ bvhProperties, influenceAreaPointFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottageWallsPointFullStandard15", cottageWallsTriangles, vector{&rayCasterPointFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottageWallsPointFullStandard15", scene.buildAndTraverse());
			}
			...
			// === CottageWalls scene: 1 point influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFull45{ bvhProperties, influenceAreaPointFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottageWallsPointFullStandard45", cottageWallsTriangles, vector{&rayCasterPointFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottageWallsPointFullStandard45", scene.buildAndTraverse());
			}
			...
			// === CottageWalls scene: 1 point influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFullOblique{ bvhProperties, influenceAreaPointFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CottageWallsPointFullStandardOblique", cottageWallsTriangles, vector{&rayCasterPointFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CottageWallsPointFullStandardOblique", scene.buildAndTraverse());
			}
		}

		// === Crowd scene: 1 point full influence area ===
		if constexpr (POINT_FULL_STANDARD and_or CROWD_SCENE) {
			// === Crowd scene: 1 point influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PointInfluenceArea influenceAreaPointFullParallel{ Pov{{2,2.2,-8}, {0,0,1}, 130, 45}, 30, 1, 10000 };
				PointRayCaster rayCasterPointFullParallel{ influenceAreaPointFullParallel }; rayCasterPointFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPointFullParallel{ bvhProperties, influenceAreaPointFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CrowdPointFullStandardParallel", crowdTriangles, vector{&rayCasterPointFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CrowdPointFullStandardParallel", scene.buildAndTraverse());
			}
			...
			// === Crowd scene: 1 point influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFull15{ bvhProperties, influenceAreaPointFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CrowdPointFullStandard15", crowdTriangles, vector{&rayCasterPointFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CrowdPointFullStandard15", scene.buildAndTraverse());
			}
			...
			// === Crowd scene: 1 point influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFull45{ bvhProperties, influenceAreaPointFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CrowdPointFullStandard45", crowdTriangles, vector{&rayCasterPointFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CrowdPointFullStandard45", scene.buildAndTraverse());
			}
			...
			// === Crowd scene: 1 point influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFullOblique{ bvhProperties, influenceAreaPointFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "CrowdPointFullStandardOblique", crowdTriangles, vector{&rayCasterPointFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("CrowdPointFullStandardOblique", scene.buildAndTraverse());
			}
		}

		// === Random100 scene: 1 point full influence area ===
		if constexpr (POINT_FULL_STANDARD and_or RANDOM100_SCENE) {
			// === Random100 scene: 1 point influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PointInfluenceArea influenceAreaPointFullParallel{ Pov{{5,5,-4}, {0,0,1}, 90, 90}, 20, 1, 10000 };
				PointRayCaster rayCasterPointFullParallel{ influenceAreaPointFullParallel }; rayCasterPointFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPointFullParallel{ bvhProperties, influenceAreaPointFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random100PointFullStandardParallel", random100Triangles, vector{&rayCasterPointFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random100PointFullStandardParallel", scene.buildAndTraverse());
			}
			...
			// === Random100 scene: 1 point influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFull15{ bvhProperties, influenceAreaPointFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random100PointFullStandard15", random100Triangles, vector{&rayCasterPointFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random100PointFullStandard15", scene.buildAndTraverse());
			}
			...
			// === Random100 scene: 1 point influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFull45{ bvhProperties, influenceAreaPointFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random100PointFullStandard45", random100Triangles, vector{&rayCasterPointFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random100PointFullStandard45", scene.buildAndTraverse());
			}
			...
			// === Random100 scene: 1 point influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFullOblique{ bvhProperties, influenceAreaPointFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random100PointFullStandardOblique", random100Triangles, vector{&rayCasterPointFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random100PointFullStandardOblique", scene.buildAndTraverse());
			}
		}

		// === Random1000 scene: 1 point full influence area ===
		if constexpr (POINT_FULL_STANDARD and_or RANDOM1000_SCENE) {
			// === Random1000 scene: 1 point influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PointInfluenceArea influenceAreaPointFullParallel{ Pov{{5,5,-4}, {0,0,1}, 90, 90}, 20, 1, 10000 };
				PointRayCaster rayCasterPointFullParallel{ influenceAreaPointFullParallel }; rayCasterPointFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPointFullParallel{ bvhProperties, influenceAreaPointFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random1000PointFullStandardParallel", random1000Triangles, vector{&rayCasterPointFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random1000PointFullStandardParallel", scene.buildAndTraverse());
			}
			...
			// === Random1000 scene: 1 point influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFull15{ bvhProperties, influenceAreaPointFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random1000PointFullStandard15", random1000Triangles, vector{&rayCasterPointFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random1000PointFullStandard15", scene.buildAndTraverse());
			}
			...
			// === Random1000 scene: 1 point influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFull45{ bvhProperties, influenceAreaPointFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random1000PointFullStandard45", random1000Triangles, vector{&rayCasterPointFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random1000PointFullStandard45", scene.buildAndTraverse());
			}
			...
			// === Random1000 scene: 1 point influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFullOblique{ bvhProperties, influenceAreaPointFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "Random1000PointFullStandardOblique", random1000Triangles, vector{&rayCasterPointFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("Random1000PointFullStandardOblique", scene.buildAndTraverse());
			}
		}

		// === Sponza scene: 1 point full influence area ===
		if constexpr (POINT_FULL_STANDARD and_or SPONZA_SCENE) {
			// === Sponza scene: 1 point influence area parallel to x covering all the scene. 1 ray caster relative to the influence area. ===
			{
				PointInfluenceArea influenceAreaPointFullParallel{ Pov{{-9,11,-1}, {1,0,0}, 70, 50}, 50, 1, 10000 };
				PointRayCaster rayCasterPointFullParallel{ influenceAreaPointFullParallel }; rayCasterPointFullParallel.generateRays(rng, 1000, true);
				Bvh bvhPointFullParallel{ bvhProperties, influenceAreaPointFullParallel, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFullParallel) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SponzaPointFullStandardParallel", sponzaTriangles, vector{&rayCasterPointFullParallel}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SponzaPointFullStandardParallel", scene.buildAndTraverse());
			}
			...
			// === Sponza scene: 1 point influence area 15 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFull15{ bvhProperties, influenceAreaPointFull15, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFull15) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SponzaPointFullStandard15", sponzaTriangles, vector{&rayCasterPointFull15}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SponzaPointFullStandard15", scene.buildAndTraverse());
			}
			...
			// === Sponza scene: 1 point influence area 45 degrees covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFull45{ bvhProperties, influenceAreaPointFull45, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFull45) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SponzaPointFullStandard45", sponzaTriangles, vector{&rayCasterPointFull45}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SponzaPointFullStandard45", scene.buildAndTraverse());
			}
			...
			// === Sponza scene: 1 point influence area oblique covering all the scene. 1 ray caster relative to the influence area. ===
			{
				...
				Bvh bvhPointFullOblique{ bvhProperties, influenceAreaPointFullOblique, bvhStrategies::computeCostSah, bvhStrategies::chooseSplittingPlanesLongest, bvhStrategies::shouldStopThresholdOrLevel, "point" };
				TopLevelOctree topLevel{ octreeProperties, fallbackBvh, std::move(bvhPointFullOblique) };
				auto scene = TestScene{ string(RESULTS_DIRECTORY) + "SponzaPointFullStandardOblique", sponzaTriangles, vector{&rayCasterPointFullOblique}, std::move(topLevel), topLevelAnalyzer };
				csvTraversal.addAnalysis("SponzaPointFullStandardOblique", scene.buildAndTraverse());
			}
		}
	};
