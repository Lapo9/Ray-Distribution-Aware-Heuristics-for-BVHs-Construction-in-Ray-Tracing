...
- PAH is much more precise than SAH at predicting the cost of a BVH.
- Projected area culling
- PAH and SPFH produce higher quality BVHs than SAH and LSPH. We tried this with these BVH properties: xyz. We will now analyze PAH and SPFH in isolation.
- PAH doesn't produce better BVHs than SAH if LSPH is used. This happens because, in case the splitting plane is not built to minimize overlapping, PAH produces a very big child and a very small one, and they completely overlap. PAH is also far slower than SAH.
- SPFH produces higher quality BVHs than LSPH with both SAH ans PAH. We used these properties for the thresholds: xyz. With these properties, for each node, on average the splitting phase is done x.y times. This number is higher in scenarios where there isn't a clear cutting orientation because the fallback is often used. For this same reason, the gain of SPFH over LSPH is reduced for oblique and 45 test cases. SPFH is not slower than LSPH.
- Breakeven hit percentage
- The projected overlapping is a good metric to explain why SPFH works. We compute how much 2 children overlap (explain formula). We decided to compute the overlapping at different levels of the BVH. At high levels SPFH produces lower overlapping percentages than LSPH. The overlapping is directly correlated with BVH quality. At lower levels the overlapping is not reduced. This means we can use SPFH on higher levels and then fall back to LSPH.
