...
\begin{subequations}
	\begin{align*}[left=\empheqlbrace]
		min.x \leq P.x \leq max.x \\
		min.y \leq P.y \leq max.y \\
		min.z \leq P.z \leq max.z
	\end{align*}
\end{subequations}
...
$P'' = \frac{P'}{P'.w}$ $P''$ is now in normalized device coordinates (NDC) space, where the frustum is an axis-aligned bounding box that extends from $\langle -1, -1, -1 \rangle$ to $\langle 1, 1, 1 \rangle$ \footnote{Based on the convention used, it is possible that the AABB in NDC space has a different size. For example, it is common an AABB extending from $\langle -1, -1, 0 \rangle$ to $\langle 1, 1, 1 \rangle$}.
