...
- PAH is extremely accurate
- PAH is more accurate with plane influence areas
- PAH + SPFH produce higher quality BVHs when rays are perpendicular to a cartesian plane (and it is better with plane influence areas)
- Hit percentage impacts on our method, due to fallback searches --> it is possible to compute the minimum hit percentage to use our heuristics effectively
- PAH without SPFH is not good
- SPFH without PAH ?????????????
- SPFH actually helps at reducing overlapping, but only at higher levels
