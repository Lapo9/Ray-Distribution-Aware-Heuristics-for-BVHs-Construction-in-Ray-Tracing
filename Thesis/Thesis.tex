...
In this appendix we explain how we implemented the uniform random sampling from a sphere cap, given an angle $\alpha$ and a radius $R$ \ref{fig:sphere_cap}. This method is used to implement the circular sphere cap ray caster \ref{ssec:ray_caster}.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth*\real{0.4}]{Images/TODO.png} 
    \caption{A sphere cap of angle $\alpha$ and radius $R$.}
    \label{fig:sphere_cap}
\end{figure}

The idea is to rotate a vector along two axes:
\begin{enumerate}
	\item First, we rotate the vector along one of the axes perpendicular to it. We call this \textit{yaw}, and its values range from $0$ to $\alpha$.
	\item Then we rotate the new vector we got along the axis parallel to the original vector. We call this \textit{roll}, and its values range from $0$ to $2\pi$.
\end{enumerate}

The issue with this method is how we sample the first random rotation, the yaw. Indeed we cannot use uniform sampling. If we used uniform sampling, we would get more samples in the region close to the center of the sphere cap, and fewer and fewer samples the more we get far from it. The reason behind this can be easily intuited by looking at figure \ref{fig:concentric_circles}: the more we step away from the center of the cap, the more the circles get bigger. If we used uniform sampling, there would be the same probability for the random sample to be placed in any of the circles. However, this means that inner circles would have a bigger sample-density than outer ones.

To solve this issue, we must sample the yaw with a PDF proportional to the rate of increase of the circumferences. In other words, since we want to keep the sample-density constant, if a circumference is twice as long as another one, it must receive twice the samples.

Given a sphere of ray $R$ and oriented toward $\overrightarrow{u}$, the length of the circumference passing through a the point of the sphere intersected by a vector forming with $\overrightarrow{u}$ an angle $\alpha$, can be obtained by using this formula:

$$C(\alpha) = R \cdot sin(\alpha) \cdot 2\pi$$
...
\begin{enumerate}
	\item Normalize the function ($C(\alpha)$ in this case) to obtain the PDF.
...
In our case, the first thing to do is to compute the normalization factor of $C(\alpha)$. To obtain it is is sufficient to integrate the function from $0$ to $\alpha$:

$$k_{norm}(a)=\int_{0}^{\alpha} C(a) da$$
...
$$k_{approx}(a) = 2.85\cdot a^{1.81}$$

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth*\real{0.3}]{Images/norm_factor_approx.png} 
...
$$pdf(x) = \frac{C(x)}{f_approx(\alpha)}$$

The next step is to compute the CDF from the PDF. The CDF is obtained by integrating the PDF. In our case the CDF has form:

$$-\frac{2\pi \cdot R}{k_{approx}} \cdot cos(x) + \frac{2\pi \cdot R}{k_{approx}}$$

The third step is to compute the inverted CDF, which in our case is:

$$icdf(x) = arccos\Bigg(\frac{\frac{2\pi \cdot R}{k_{approx}}-x}{\frac{2\pi \cdot R}{k_{approx}}}\Bigg)$$

Now that we obtained the ICDF, we can get uniform random samples $X_{uniform}$, and obtained the random samples according to the PDF in this way:
