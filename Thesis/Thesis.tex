...
\subsection{PAH and SPFH versus SAH and LSPH}
With the tests presented in this section, we compare the building and traversal of BVHs built with the two heuristics we propose in this work, to BVHs built with the-state-of-the art heuristics, namely surface area heuristic (SAH) and longest splitting plane heuristic (LSPH). 
...
\begin{itemize}
	...
	\item How much faster, in percentage, \textit{our full structure} is faster than the state-of-the-art BVH. With the term \textit{our full structure} we refer to the process of traversing the PAH BVH, and, possibly, traversing the global fallback BVH (chapter \ref{ch:multi_influence_areas}), in case the search in the PAH BVH haven't found any intersection: $faster \; full = \frac{cost_{SAH+LSPH}-(cost_{PAH+SPFH}+cost_{fallback})}{cost_{PAH+SPFH}+cost_{fallback}}$.
...
As it is possible to observe in the chart \ref{fig:pah_without_fallback_faster_chart}, our heuristics on average produce a faster BVH only in the case of influence areas with rays that are parallel or \textit{almost parallel} (15 degrees) to one cartesian axis. 
...
- PAH + SPFH produce higher quality BVHs when rays are perpendicular to a cartesian plane (and it is better with plane influence areas) (parallel 25\% better, 15 7\%, 45 -13\%, oblique -9\%)
	- Graph of PAH without fallback faster percentage of summary plane and point SAH + SPFH
...
- Hit percentage impacts on our method, due to fallback searches --> it is possible to compute the minimum hit percentage to use our heuristics effectively (breakeven hit \%)
	- Graph of summary hit percentage and PAH faster percentage of plane and point SAH + SPFH
