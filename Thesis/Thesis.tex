...
In this chapter we will show how it is possible to deal with a scene where there are multiple localized ray distributions. We will first introduce the the problem in detail and with visual examples, then we will define some properties of the required solution, and eventually we will show how a scene with multiple ray distributions can be efficiently traversed by a ray. In this chapter we won't explain all the implementation details of the solution we propose, that will instead be discussed in section \ref{TODO}.
